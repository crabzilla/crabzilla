package io.github.crabzilla.handler.internal
...
import io.github.crabzilla.handler.CommanderException
import io.github.crabzilla.handler.TargetStream
import io.github.crabzilla.handler.internal.StreamWriter.Companion.SQL_GET_STREAM
...
internal data class StreamSnapshot<S : Any>(
  val streamId: Int,
  val state: S,
  val version: Int,
...
internal class StreamRepository<S : Any, E : Any>(
...
internal class StreamWriter<S : Any, E : Any>(
  private val conn: SqlConnection,
  private val targetStream: TargetStream,
  ...
  private val log = LoggerFactory.getLogger("${StreamWriter::class.java.simpleName}-${targetStream.stateType}")
    ...
    return conn
      ...
      .compose { pgRow ->
        if (pgRow.first().getBoolean("locked")) {
          ...
          Future.failedFuture(CommanderException.StreamCantBeLockedException("Stream $streamId can't be locked"))
        }
      }
  }
