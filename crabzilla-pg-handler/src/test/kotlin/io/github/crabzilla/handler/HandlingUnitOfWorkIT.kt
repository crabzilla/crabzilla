...
class HandlingUnitOfWorkIT : AbstractCommandIT() {
  ...
  fun `it can handle 2 commands within a transaction`(
    ...
    context
      .withinTransaction { tx ->
        crabzillaHandler.handleWithinTransaction(tx, targetStream1, cmd1)
          .compose { crabzillaHandler.handleWithinTransaction(tx, targetStream2, cmd2) }
      }
  ...
  fun `it can rollback when handling 2 commands within a transaction `(
    ...
    context
      .withinTransaction { tx ->
        crabzillaHandler.handleWithinTransaction(tx, targetStream1, cmd1)
          .compose { crabzillaHandler.handleWithinTransaction(tx, targetStream1, cmd2) }
          .onComplete {
            testRepository.scanEvents(0, 1000)
              .onFailure { tc.failNow(it) }
              .onSuccess { list ->
