...
internal class ProjectionFromHzRbIT {
  ...
  private val mgr = HazelcastClusterManager()

  private lateinit var vertx: Vertx
  private lateinit var client: WebClient

  companion object {
    private val log = LoggerFactory.getLogger(ProjectionFromHzRbIT::class.java)
    ...
    Vertx.clusteredVertx(options) { res: AsyncResult<Vertx> ->
      ...
      getConfig(vertx, "./../example1.env")
        ...
        .onSuccess { config: JsonObject ->
          ...
          ProjectionFromHzRbVerticle.hz = mgr.hazelcastInstance // TODO wont work on a cluster
          deploy(vertx, ProjectionFromHzRbVerticle::class.java.name, deploymentOptions)
            .onComplete {
  ...
  fun a3(tc: VertxTestContext) {
    ...
    client.post("/commands/customer/1/$commandWithoutType")
      .`as`(BodyCodec.jsonObject())
      .expect(ResponsePredicate.SC_BAD_REQUEST)
      .sendJson(invalidCommand, tc.succeeding { response: HttpResponse<JsonObject> ->
        tc.verify {
