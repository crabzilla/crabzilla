...
internal class DbSnapshotAcceptanceIT {
  ...
  companion object {
    private val log = LoggerFactory.getLogger(DbSnapshotAcceptanceIT::class.java)
    init {
  ...
  fun setup(tc: VertxTestContext, vertx: Vertx) {
    getConfig(vertx, "./../example1.env")
      .onSuccess { config: JsonObject ->
        ...
        deploy(vertx, DbSnapshotVerticle::class.java.name, deploymentOptions)
          .onComplete {
