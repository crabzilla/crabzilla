...
//import java.util.*
//
//internal class PgcUowRepo2(private val pgPool: PgPool,
//                           private val jsonFunctions: Map<String, EntityJsonAware<out Entity>>)
//  : UnitOfWorkRepository {
//
//  companion object {
//
//    internal val log = LoggerFactory.getLogger(PgcUowRepo2::class.java)
//
...
//    private const val ENTITY_ID = "ar_id"
//    private const val ENTITY_NAME = "ar_name"
...
//
//      val row = rows.first()
//      val entityName = row.getString(ENTITY_NAME)
//      val jsonAware: EntityJsonAware<out Entity>? = if (jsonFunctions.containsKey(entityName)) {
//          jsonFunctions[entityName]
//        } else {
//          aHandler.handle(Future.failedFuture(IllegalStateException("JsonAware for $entityName wasn't found")))
//          return@preparedQuery
//      }
//
//      val commandName = row.getString(CMD_NAME)
//      val commandAsJson = row.getJson(CMD_DATA).value().toString()
//      val command = try { jsonAware!!.cmdFromJson(commandName, JsonObject(commandAsJson)) }
...
//      val uow = UnitOfWork(row.getString(ENTITY_NAME), row.getInteger(ENTITY_ID),
...
//        val commandAsJson = row.getJson(CMD_DATA).value().toString()
//        val command =
//          try { jsonAware!!.cmdFromJson(commandName, JsonObject(commandAsJson)) } catch (e: Exception) { null }
//        if (command == null) {
//          aHandler.handle(Future.failedFuture("error when getting command $commandName from json "))
//          return@preparedQuery
//        }
...
//              val eventName = jsonObject.getString(EVENT_NAME)
//              val eventJson = jsonObject.getJsonObject(EVENTS_JSON_CONTENT)
//              jsonAware!!.eventFromJson(eventName, eventJson)
//            }
