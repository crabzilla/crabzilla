...
class HandlingConcurrencyIT : AbstractCommandIT() {
  ...
  @Test
  fun `when many concurrent commands against same version, just one will succeed`(
    vertx: Vertx,
    tc: VertxTestContext,
  ) {
      ...
      .onSuccess {
        vertx.executeBlocking<Void> { promise ->
          ...
          for (i: Int in 1..concurrencyLevel) {
            callables.add(Callable { commandComponent.handle(id, cmd2) { currentVersion -> currentVersion == 1 } })
          }
    ...
    commandComponent.handle(id, cmd)
      ...
      .onSuccess {
        vertx.executeBlocking<Void> { promise ->
          ...
          val callables = mutableSetOf<Callable<Future<EventMetadata>>>()
          for (i: Int in 1..concurrencyLevel) {
            callables.add(Callable { commandComponent.handle(id, cmd2) })
          }
          val futures = executorService.invokeAll(callables)
          executorService.awaitTermination(3, TimeUnit.SECONDS)
          val failures = futures.map { it.get() }.filter { it.failed() }
