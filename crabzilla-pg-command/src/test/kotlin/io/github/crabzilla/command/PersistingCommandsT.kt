...
class PersistingCommandsT : AbstractCommandHandlerIT() {
  @Test
  fun `it can persist 1 command`(
    ...
    commandHandler.handle(targetStream, cmd)
  ...
  fun `it can persist 1 command with metadata`(
    ...
    commandHandler.handle(targetStream, cmd, cmdMetadata)
      .compose { testRepository.getCommands() }
  ...
  fun `it can persist 2 commands`(
    ...
    commandHandler.handle(targetStream, cmd1)
      .compose { commandHandler.handle(targetStream, cmd2) }
      .compose { testRepository.getCommands() }
      .compose { commands -> testRepository.getEvents(0L, 10).map { Pair(commands, it) } }
      .onFailure { tc.failNow(it) }
      .onSuccess { pair ->
  ...
  fun `it can persist 1 command without body`(
    ...
    commandHandler = CommandHandlerImpl(context, customerConfig.copy(commandSerDer = null))
  ...
  fun `it can skip persisting command`(
    ...
    commandHandler = CommandHandlerImpl(context, customerConfig.copy(persistCommands = false))
