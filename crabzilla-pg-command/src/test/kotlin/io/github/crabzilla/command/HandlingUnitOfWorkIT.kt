...
class HandlingUnitOfWorkIT : AbstractCommandHandlerIT() {
  ...
  fun `it can handle 2 commands within a transaction`(
    ...
    commandHandler
      .withinTransaction { tx ->
        commandHandler.handleWithinTransaction(tx, targetStream1, cmd1)
          .compose { commandHandler.handleWithinTransaction(tx, targetStream2, cmd2) }
      }
  ...
  fun `it can rollback when handling 2 commands within a transaction `(
    ...
    commandHandler
      .withinTransaction { tx ->
        commandHandler.handleWithinTransaction(tx, targetStream1, cmd1)
          .compose { commandHandler.handleWithinTransaction(tx, targetStream1, cmd2) }
          .onComplete {
            testRepository.getEvents(0, 1000)
              .onFailure { tc.failNow(it) }
              .onSuccess { list ->
