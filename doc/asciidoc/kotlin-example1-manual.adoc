...
database. This is called _persistence ignorance_. Although it is agnostic about relational persistence, this module
is aware of the JSON format needed to persist its commands, events, units of work, etc since its only required
dependency is crabzilla-core, witch depends on https://github.com/FasterXML/jackson-module-kotlin[jackson-module-kotlin].
...
An possibly blocking and possibly NON side effect free function. Although not being side effect free (throws an exception),
command handler verticles will wrap the result of this function into a result type something like https://github.com/crabzilla/crabzilla/blob/master/crabzilla-core/src/main/java/io/github/crabzilla/core/entity/EntityCommandResult.kt[EntityCommandResult]
[source,kotlin]
.CustomerFunctions.kt
----
